...
class HeavyHexCircuitBuilder:
    ...
    def _add_x_circuit(self):
        ...
        [("TL", 1), ("BL", 0)],#step 1
        [("TL", 2), ("BL", 1)],#step 2
        [("TL", 3), ("BL", 2), ("TR", 3)],#step 3
        [("BL", 3), ("TR", 0), ("BR", 3)],#step 4
        [("TR", 1), ("BR", 0)],#step 5
        [("TR", 2), ("BR", 1)],#step 6
        [("BR", 2)],#step 7
        ]
...
class ConstructDetectors:
    ...
    def init_detectors(self, basis: str):
        ...
        for tile in group:
            keys = {gen.AtLayer(tile.measure if basis == 'X' else tile.flags[0], f'{basis.lower()}_round_{self.step}')}
            if keys:
            ...
            for group in groups:
                ...
                for tile in group:
                    if not is_end:
                        ...
                        if self.step < 2:
                            keys = keys if basis == 'X' else set() # for step 0 and step 1, only current round step can be used as the detector since the previous round step is not exist for step 0 and the error caused by without reset cannot cancel out in step 1
                        elif gen.AtLayer(tile.measure, f'x_round_{self.step - 2}') in self.recorded_measurement_keys:
                            # last round step in the case with no reset use -2, with reset use -1
                            keys.add(gen.AtLayer(tile.measure, f'x_round_{self.step - 2}')) 
                        else:
                        ...
                        if gen.AtLayer(tile.measure, f'x_round_{self.step - 1}') in self.recorded_measurement_keys:
                            keys_end.add(gen.AtLayer(tile.measure, f'x_round_{self.step - 1}'))
    ...
    def general_z_detectors(self,basis: str, is_end: bool = False, org_tile_group_z: List[Tile]=[]):
        ...
        if self.step < 2:
            ...
            check_steps = [self.step, self.step - 1, self.step - 2, self.step - 3]
        else:
            check_steps = [self.step - 1 , self.step - 2] if self.step == 2 else [self.step - 1 , self.step - 2, self.step - 3] if self.step == 3 else [self.step - 1, self.step - 2, self.step - 3, self.step - 4]

        def set_previous_keys(flags: set[complex], check_steps: list[int]) -> List[Set[gen.AtLayer]]:
            # Always return exactly 4 sets; missing/invalid rounds are represented by empty sets.
            normalized_steps = check_steps[:4] + [None] * (4 - len(check_steps))
            return [
                {gen.AtLayer(f, f"z_round_{step}") for f in flags} if isinstance(step, int) else set()
                for step in normalized_steps
            ]

        for group in groups:
            flags = {tile.flags[0] for tile in group}
            prev_keys = set_previous_keys(flags, check_steps)
            recorded = self.recorded_measurement_keys
            have_0   = prev_keys[0] <= recorded           # all flags present at step - 1
            have_1 = len(prev_keys) > 1 and (prev_keys[1] <= recorded)  # all flags present at step - 2
            keys  = {gen.AtLayer(flag, f"z_round_{self.step}") for flag in flags}
            if not is_end:
                if self.step < 2:
                    keys = keys if basis == "Z" else set()
                else:
                # warnings.warn(f"previous key has length {len(prev_keys)} for step {self.step}")
                    if have_0 and have_1:
                        keys.update(prev_keys[1]) # if have both step - 1 and step - 2, then step - 2 can be used as a part of detector since the error caused by without reset cancle out in this case
                    elif have_0:
                        if prev_keys[2]:
                            keys.update(prev_keys[2])
                    elif have_1 and not have_0:
                        if prev_keys[3]:
                            keys.update(prev_keys[3])
                    else:
                        keys = keys if basis == "Z" else set()
                ...
                if have_0 and have_1:
                    keys_end.update(prev_keys[0])
                    keys_end.update(prev_keys[1])
                elif have_0 and not have_1:
                    keys_end.update(prev_keys[0])
                    keys_end.update(prev_keys[2])
                elif have_1 and not have_0:
                    keys_end.update(prev_keys[1])
                    keys_end.update(prev_keys[3])
                else:
                    keys_end = set()
                    warnings.warn(f"No previous step keys found for z-detectors at tail {group} at timestep {self.step} ")
                if keys_end:
